...
class LogisticDemo:
    ...
    def __init__(self, export_dir="exports"):
        ...
        def slider(label, lo, hi, initial, row, step):
            control = Slider(self.fig.add_axes([0.15, row, 0.37, 0.024]), label,
                             lo, hi, valinit=initial, valstep=step, valfmt="%1.3g")
            control.drawon = False  # One shared renderer; no full redraw per widget.
            control.vline.set_visible(False)
            return control

        self.sliders = {
            "weight": slider("Gewicht w", -10, 10, 1, 0.387, 0.05),
            "bias": slider("Bias b", -6, 6, 0, 0.318, None),
            "alpha": slider("Steilheit alpha", 0.1, 100, 5, 0.387, 0.1),
            "threshold": slider("Schwelle t", -5, 5, -3, 0.318, 0.05),
            "x": slider("Eingabe x", -10, 10, 0, 0.249, None),
        ...
        mode_ax = self.fig.add_axes([0.64, 0.315, 0.155, 0.11])
        mode_ax.set_title("Modus", loc="left", fontsize=11)
        self.mode = RadioButtons(mode_ax, [self.FREE, self.THRESHOLD], useblit=False)
        preset_ax = self.fig.add_axes([0.835, 0.165, 0.14, 0.26])
        preset_ax.set_title("Beispiele laden", loc="left", fontsize=11)
        self.presets = RadioButtons(preset_ax, self.PRESETS, useblit=False)
        check_ax = self.fig.add_axes([0.64, 0.205, 0.155, 0.075])
        self.checks = CheckButtons(check_ax, ["Referenz H_t"], [True], useblit=False)
        self.fig.text(0.64, 0.186, "Nur im Schwellenmodus", fontsize=9, color="0.35")
        self.buttons = {}
    ...
    def update(self, _=None):
        ...
        show_hard = locked and self.checks.get_status()[0]
    ...
    def apply_preset(self, name):
        ...
        try:
            ...
            self._updating = False
        self.status.set_text(f"Beispiel geladen: {name}. Regler ändern diesen Ausgangszustand.")
        self.update()
    ...
    def reset(self, _=None):
        ...
        try:
            ...
            if not self.checks.get_status()[0]:
